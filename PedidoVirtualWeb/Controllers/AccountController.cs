        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    {
                        if (!PedidoBase.Usuario.Equals("") && !PedidoBase.Usuario.Equals(model.Email))
                        {
                            foreach (PedidoItem pedidoItem in PedidoBase.Carrinho)
                            {
                                db.Items.Where(I => I.ItemId == pedidoItem.ItemId).FirstOrDefault().Estoque += pedidoItem.Quantidade;
                            }
